---
layout: default
title: Home - The Wonky Sprout
---
<div class="card-grid">
    <a href="/daily-tracker" class="card">
        <h2>📊 Daily Tracker</h2>
        <p>Today's protocols and metrics.</p>
    </a>
    <a href="/sop-vault" class="card bg-soft-blue">
        <h2>📚 The Vault</h2>
        <p>SOPs & guides.</p>
    </a>
    <a href="/command-center" class="card">
        <h2>⚙️ Command Center</h2>
        <p>External tools and resources.</p>
    </a>
    <a href="/checklists" class="card bg-soft-green">
        <h2>✅ Checklists</h2>
        <p>Quick protocols for daily tasks.</p>
    </a>
    <a href="/little-sprouts" class="card">
        <h2>👨‍👧‍👦 Little Sprouts</h2>
        <p>Protocols for the kids.</p>
    </a>
    <a href="/2025-11-09_M1_PAGE_manifesto-v2" class="card">
        <h2>📜 Manifesto</h2>
        <p>The philosophy behind the Wonky Sprout.</p>
    </a>
    <a href="/2025-11-08_M1_PAGE_about-homepage" class="card bg-soft-green">
        <h2>ℹ️ About</h2>
        <p>More about this project.</p>
    </a>
    <a href="/settings" class="card bg-soft-blue">
        <h2>🔧 Settings</h2>
        <p>Site and user settings.</p>
    </a>
    <a href="/games" class="card">
        <h2>🎮 Games</h2>
        <p>Games and activities.</p>
    </a>
    <a href="/workspace" class="card">
        <h2>💼 Workspace</h2>
        <p>Tools and links for work.</p>
    </a>
</div>
